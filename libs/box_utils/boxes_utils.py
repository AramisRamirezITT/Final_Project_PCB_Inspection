...
def clip_boxes_to_img_boundaries(decode_boxes, img_shape):
    ...
    with tf.name_scope('clip_boxes_to_img_boundaries'):
        ...
        xmin = tf.maximum(tf.minimum(xmin, img_w), 0.)
        ymin = tf.maximum(tf.minimum(ymin, img_h), 0.)

        xmax = tf.maximum(tf.minimum(xmax, img_w), 0.)
        ymax = tf.maximum(tf.minimum(ymax, img_h), 0.)
